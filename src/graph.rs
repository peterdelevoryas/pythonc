...
impl Graph {
    ...
    fn add_referenced_variables(&mut self, inst: &vasm::Inst) {
        ...
        match *inst {
            ...
            Mov(lval, rval) | Add(lval, rval) | Cmp(lval, rval) | Or(lval, rval) |
            And(lval, rval) => {
                self.add_lval(lval);
                self.add_rval(rval);
            }
            ...
            If(lval, ref then, ref else_) => {
                self.add_lval(lval);
                for inst in &then.insts {
                    self.add_referenced_variables(inst)
                }
                for inst in &else_.insts {
                    self.add_referenced_variables(inst)
                }
            }
    ...
    fn add_edges(&mut self, live_set: LiveSet) {
        ...
        match live_set {
                ...
                match *inst {
                    ...
                    Call(_) | CallIndirect(_) => {
                        let caller_save_registers =
                            &[::vasm::Reg::EAX, ::vasm::Reg::ECX, ::vasm::Reg::EDX];
                    ...
                    While(_cond, ref _comp, ref _body) => {
                        panic!("encountered While in wrong place")
                    }
                    JmpLabel(_) => unimplemented!(),
                    JeqLabel(_) => unimplemented!(),
                    Sub(_, _) => unimplemented!(),
            ...
            LiveSet::While { inst, header, body, .. } => {
                match *inst {
                    While(_, _, _) => {}
                    _ => panic!(),
                }
    ...
    pub fn run_dsatur(&mut self) -> DSaturResult {
        ...
        loop {
            ...
            let (u, r) = if let Some(&u) = uncolored_nodes.iter().max_by_key(
                |&var| self.saturation(*var),
            )
            {
                let diff: HashSet<Reg> = registers
