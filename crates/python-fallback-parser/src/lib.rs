...
fn destructure_module(module: PyObject, py: Python) -> Result<Module, ParseError> {
    ...
    let py_type_string: &PyString = match py_type.cast_as(py) {
        Ok(ps) => Ok(ps),
        _ => Err(String::from("Failed to convert class type attribute to Python String")),
    }?;
    if py_type_string.to_string(py)? != "Module" {
        return Err(ParseError(String::from("Module does not refer to Python Module Object")));
    }
    ...
    let stmts: &PyList = match attr.cast_as(py) {
        Ok(pl) => Ok(pl),
        _ => Err(String::from("Failed to convert module.node.nodes to list type")),
    }?;
...
fn destructure_stmt(stmt: PyObject, py: Python) -> Result<Statement, ParseError> {
    ...
    let py_type_string: &PyString = match py_type.cast_as(py) {
        Ok(ps) => Ok(ps),
        _ => Err(format!("{:?}: Couldn't convert class type attribute to PyString", stmt)),
    }?;
    ...
    let rstmt = match py_type_string.to_string(py)?.borrow() {
        ...
        "Discard" => {
            Statement::Expression(destructure_expr(stmt.getattr(py, "expr")?, py)?)
        },
        "Printnl" => {
...
fn destructure_assignment(assig : PyObject, py : Python) -> Result<Statement, ParseError>
{
    ...
    let lvalues: &PyList = match nodes.cast_as(py) {
        Ok(pl) => pl,
        _ => { return Err(ParseError(format!("{:?}: Could not convert assignment nodes to list", assig))); },
    };

    if lvalues.len(py) <= 0 {
        return Err(ParseError(String::from("Assignment does not have at least one node")));
    }
    ...
    let target_name_pys: &PyString = match target_name_pyo.cast_as(py) {
        Ok(ps) => ps,
        _ => { return Err(ParseError(format!("{:?}: Could not convert assigned name to list", target_name_pyo))); },
    };
...
fn destructure_expr(expr: PyObject, py: Python) -> Result<Expression, ParseError>
{
    ...
    let py_type_string: &PyString = match py_type.cast_as(py) {
        Ok(ps) => Ok(ps),
        _ => Err(format!("{:?}: Couldn't convert class type attribute to PyString", expr)),
    }?;
    ...
    let rexpr = match py_type_string.to_string(py)?.borrow() {
        "CallFunc" => {
            ...
            let func_pstr : &PyString = match func.cast_as(py) {
                Ok(ps) => ps,
                _ => { return Err(ParseError(format!("{:?}: could not convert name to PyString", func))); },
            };

            let func_cowstr = func_pstr.to_string(py)?;
            let func_str: &str = func_cowstr.borrow();
            if func_str != "input" {
                return Err(ParseError(format!("{:?}: unhandled function name", func_str)))
            }

            let args = expr.getattr(py, "args")?;
            let arg_plist : &PyList = match args.cast_as(py) {
                Ok(pl) => pl,
                _ => { return Err(ParseError(format!("{:?}: could not convert args to PyList", args))); },
            };
        ...
        "Const" => {
            ...
            if tname != "int" {
                return Err(ParseError(String::from("Only integer constants are supported")));
            }
        ...
        "Name" => {
            ...
            let name_pstr : &PyString = match name.cast_as(py) {
                Ok(ps) => ps,
                _ => { return Err(ParseError(format!("{:?}: failed to convert name to PyString", name))); },
            };
